...
class Authentication:
    ...
    def setup_auth_ui(self):
        ...
        if st.sidebar.button("Logout") and st.session_state.get('authenticated'):
...
class AmbulanceService:
    ...
    def update_ambulance_location(self, ambulance_id, latitude, longitude, location_name):
        try:
...
class MapUtils:
    @staticmethod
    def create_live_map(hospitals_df, ambulances):
        ...
        ambulance_data = []
        for ambulance in ambulances:
            color = [0, 0, 255] if ambulance.status == 'Available' else [255, 0, 0]
            ambulance_data.append({
                'longitude': ambulance.longitude,
                'latitude': ambulance.latitude,
                'name': ambulance.ambulance_id,
                'status': ambulance.status,
                'driver': ambulance.driver_name,
                'color': color
            })
        ...
        route_layer = pdk.Layer(
            'LineLayer',
            MapUtils._get_route_data(ambulances),
        ...
        r = pdk.Deck(
            layers=[hospital_layer, ambulance_layer, route_layer],
    ...
    @staticmethod
    def _get_route_data(ambulances):
        routes = []
        for ambulance in ambulances:
            if ambulance.route and len(ambulance.route) > 1:
                for i in range(len(ambulance.route) - 1):
                    routes.append({
                        'start': [ambulance.route[i]['lon'], ambulance.route[i]['lat']],
                        'end': [ambulance.route[i+1]['lon'], ambulance.route[i+1]['lat']]
                    })
        return routes
...
class ReferralUI:
    ...
    def display_active_referrals(self):
        ...
        if active_patients:
            ...
            for patient in active_patients:
                with st.expander(f"Actions for {patient.name}"):
    ...
    def display_patient_actions(self, patient):
        ...
        with col1:
            if st.button(f"Assign Ambulance", key=f"assign_{patient.patient_id}"):
            ...
            if st.session_state.get(f'assign_ambulance_{patient.patient_id}'):
                ...
                if available_ambulances:
                    ...
                    if st.button("Confirm Assignment", key=f"confirm_{patient.patient_id}"):
        ...
        with col2:
            if st.button("Update Status", key=f"status_{patient.patient_id}"):
            ...
            if st.session_state.get(f'update_status_{patient.patient_id}'):
                ...
                if st.button("Update", key=f"update_{patient.patient_id}"):
                    ...
                    st.rerun()
        with col3:
            if st.button("View Details", key=f"details_{patient.patient_id}"):
...
class TrackingUI:
    ...
    def display(self):
        ...
        with col1:
            self.display_live_map()
        with col2:
            self.display_ambulance_list()
            self.display_patient_tracking()
    
    def display_live_map(self):
        st.subheader("Live Tracking Map")
        hospitals_df = self.get_hospitals_data()
        ambulances = self.db.get_all_ambulances()
        map_obj = self.map_utils.create_live_map(hospitals_df, ambulances)
    ...
    def display_patient_tracking(self):
        st.subheader("Patient Transfers")
        patients = self.db.get_all_patients()
        ...
        if active_transfers:
            for patient in active_transfers:
                with st.expander(f"🚑 {patient.name}"):
                    ...
                    st.write(f"**Ambulance:** {patient.assigned_ambulance}")
                    status_progress = {
                        'Referred': 0, 'Ambulance Dispatched': 25, 'Patient Picked Up': 50,
                        'Transporting to Destination': 75, 'Arrived at Destination': 100
                    }
                    progress = status_progress.get(patient.status, 0)
                    st.progress(progress / 100)
        else:
            st.info("No active patient transfers")
    ...
    def get_hospitals_data(self):
        hospitals_data = {
            ...
            'latitude': [-0.0754, -0.0754, -0.1058, -0.1743],
            'longitude': [34.7695, 34.7695, 34.7568, 34.9169],
            'facility_type': ['Referral Hospital', 'Referral Hospital', 'Sub-County Hospital', 'Sub-County Hospital']
        }
...
class HandoverUI:
    ...
    def create_handover_form(self):
        ...
        with st.form("handover_form"):
            ...
            if st.form_submit_button("Complete Handover"):
    ...
    def communication_log(self):
        ...
        for comm in communications:
            with st.expander(f"{comm['timestamp']} - {comm['type']} to {comm['recipient']}"):
                ...
                if st.button("Resend", key=f"resend_{comm['timestamp']}"):
            ...
            if patient:
                ...
                with col2:
                    ...
                    if ambulance.latitude and ambulance.longitude:
                        # Embed Google Maps
                        maps_html = MapUtils.embed_google_maps(ambulance.latitude, ambulance.longitude)
                        st.components.v1.html(maps_html, height=400)
                    else:
                    ...
                    with col1:
                        if st.button("🚦 Start Trip", use_container_width=True):
                            ...
                            ambulance.start_time = datetime.utcnow()
                            self.db.session.commit()
                            st.success("Trip started! Navigation activated.")
                            st.rerun()
                    with col2:
            ...
            if available_patients:
                ...
                for patient in available_patients:
                    with st.expander(f"Mission: {patient.name} - {patient.condition}"):
                        st.write(f"**From:** {patient.referring_hospital}")
                        st.write(f"**To:** {patient.receiving_hospital}")
                        if st.button("Accept Mission", key=f"accept_{patient.patient_id}"):
